...
export var PROTECTIONCATEGORY =  {	

	nameTables: "protectioncategory",

	textQuery:	"CREATE TABLE IF NOT EXISTS [protectioncategory] (" +
			...
			toolbar: {
				...
				onClick: function (event) {
					if (event.target == 'add') {
						...
						row.recid = null;
						struct.push(row);
						BD.addArray(PROTECTIONCATEGORY, struct, GRIDFORM.confirmAdd);
					}
					if (event.target == 'save') {
						var selection = w2ui[PROTECTIONCATEGORY.nameTables].getChanges();
						for (var i = 0; i < selection.length; i++) {
							var record = w2ui[PROTECTIONCATEGORY.nameTables].get(selection[i].recid);
							if(record.changes){
						...
						BD.edit(PROTECTIONCATEGORY, selection, GRIDFORM.confirmEdit);
					}
					if (event.target == 'delete') {
						w2confirm('Удалить выбранные строки?', function (btn) {
							if (btn == 'Yes') {
								let selection = w2ui[PROTECTIONCATEGORY.nameTables].getSelection();
								var conditions = {};
								conditions.recid = selection;
								BD.deleteWithConditions(PROTECTIONCATEGORY, conditions, GRIDFORM.confirmDelete);						
							}
						})
					}
				}
